...
- **처리 자료 6종 / phase 5개**: ①통합조회 / ②③(세금)계산서 신고용 합계표
  (hapgye_sum 하나로 통합 — 전자세금계산서·전자계산서 × 매출·매입) /
  ④신용카드·판매(결제)대행(+정리본 엑셀) / ⑤현금영수증 매출 /
  ⑦수출실적명세서(export_sales).
  ⑥현금영수증 매입총액은 v1.0.5에서 제거(사용자 불필요 확인) — 본문의 ⑥ 언급은
  제거 전 기록. cash_common은 ⑤가 계속 사용.
...
| 12 | ① 통합조회 40초 timeout "실패"(수임은 정상) | 일부 업체(지점)는 alert "조회권한이 없습니다" → 자동 수락돼 화면 흔적 없음, 완료 신호(과세기간 칸)도 영영 안 옴 | dialogs에서 '조회권한' alert 감지(extra_keys) → 즉시 '출력 생략' 성공 처리+노란 로그, 빈 화면 검사는 예비 (v1.0.6, 라이브 검증) |
| 13 | 납부서: 번호 없이 조회돼 수임업체 전체 목록이 뜸 | 모달 오픈 직후 js_fill 값을 WebSquare 재초기화가 지움(#8과 동일 레이스) + 입력칸이 type=password라 텍스트 탐색도 실패 | ws_set_value+값검증+3회 재시도, 행 선택은 사업자번호 열(7) 일치로만 (v1.1.0) |
| 14 | 납부서: 뷰어 인쇄까지 눌렀는데 저장 다이얼로그 안 뜸 → Chrome '인쇄 실패' | 저장 다이얼로그 표시 시점에 포커스 독점 프로그램(세무사랑 등)이 전면에 있으면 다이얼로그 생성이 막힘 (사용자 실확인) | phase가 [보기]부터 1회 자동 재시도 + GUI에 사용 주의 문구, 코드로 완전 회피 불가 (v1.1.0) |
...
- 업체 명부: 인라인 추가줄(업체명/사업자번호/예정 O·X/+추가, Enter 지원).
  v1.1.1부터 **체크박스 부활** — 행 아무 곳이나 클릭 = 체크 토글(연한 파랑 배경+✔),
  **체크된 업체만 실행**(자료/납부서 공통). 큰 명부를 유지하며 그때그때 골라 실행.
  ✔ 헤더 클릭 = 전체 체크/해제 토글, 체크 삭제 버튼. checked는 clients.json에
  저장(없으면 체크 간주).
...
- 산출물: PDF는 `저장폴더\업체명\업체명_자료명_2026년1기_확정.pdf`,
  판매대행 엑셀은 `업체명_판매결제대행 매출자료조회_2026년1기_확정.xlsx`(정리본
  토글 ON, 기본) 또는 `.xls`(토글 OFF — 홈택스 원본). 파일명 끝 신고구분은
  GUI에서 고른 신고시즌(확정/예정). 엑셀도 인쇄 모드와 무관하게 항상
  업체별 폴더에 저장(v1.0.4).
...
| v1.0.2 | ⑦ 수출실적명세서 추가 |
| v1.0.3 | 중지 버튼 즉시 중단(task cancel) — 선택 업데이트로 안내 |
| v1.0.4 | 파일명 끝 신고구분(확정/예정) 추가, 판매대행 엑셀 인쇄 모드에서도 업체별 폴더 저장 |
| v1.0.5 | ⑥ 현금영수증 매입총액 phase 제거 (사용자 불필요 확인) |
| v1.0.6 | ① 통합조회 '조회권한이 없습니다' alert 예외처리 — 즉시 '출력 생략' 성공(노란 로그), 업체는 계속 진행 (해결 #12) |
| v1.1.0 | 납부서 출력 모드 — 신고내역 조회(접수증·납부서)에서 업체별 최신 1건 납부서 PDF, 파일명 템플릿({업체명}/{납부기한}+날짜토큰), 모달 재사용, 번호일치 안전장치 (해결 #13·#14, 실업체 2곳 라이브 검증) + 명부 체크박스(이미지, 행 클릭 토글·헤더 전체토글·체크만 실행) + 로그 색(실패 빨강 ✗/생략 노랑) (최신) |
...
- [x] ~~납부서 출력(payment_slip) 라이브 검증~~ — v1.1.0에서 완료 (실업체 2곳
      연속, 모달 재사용 포함). 정찰 도구 tools/peek_slip.py, 셀렉터는
      payment_slip.py 상단 상수 참조 (모달 UTERNAAZ0Z31, 그리드
      ttirnam101DVOListDes, 입력칸 type=password, 알림 info*_wframe/btn_confirm).
