...
2026-07-14 하루 동안 설계→구현→라이브 검수 8회→v1.0.0 배포까지 진행한 기록.
...
- **처리 자료 5종(phase)**: ①통합조회 / ②③(세금)계산서 신고용 합계표(하나로 통합) /
  ④신용카드·판매(결제)대행(+엑셀) / ⑤현금영수증 매출 / ⑥매입.
- **스택**: Python + Playwright(영구 프로필 Chromium) + Tkinter + PyInstaller.
  incometax_printing / ingunbi_auto에서 검증된 패턴 재사용.
...
| 시즌 | O(예정신고 함) | X(안 함) |
|---|---|---|
| 확정 | "확정" — 합계표 "1기 확정", 카드·현금 2분기만 | "예정+확정" — 합계표 "1기(예정+확정)", 카드 1~2분기, 현금 -전체- |
| 예정 | "예정" — 1분기만 | **업체 통째로 건너뜀**(pipeline, 결과 엑셀 기록) |
...
### 4.5 수임사업자전환 모달 (②③, 프레임 `UTEETZZA21_wframe_`)
...
- [ ] 배포 exe로 전체 통짜 검증 (특히 #10 재클릭 로직과 속도 최적화 이후 상태)
- [x] ④ 엑셀 토글 용도 확정(구 '월별 누계' → 후가공): `excel_summary` ON이면
      다운받은 .xls를 `agency_excel.make_summary`로 재작성 — Sheet1=정리본
      (상호 가나다순 그룹 + 승인년월순 + 상호별 금액 소계 + 총합계, 제목
      '< 1기확정 - 업체명 >'), Sheet2=원본. 산출물이 진짜 .xlsx가 됨.
      원본 .xls 읽기는 xlrd(의존성 추가). OFF면 기존대로 원본 .xls 저장.
      사용자 수작업 정리본(2026-01 예시)을 그대로 재현, 실파일 변환 검증 완료.
      합계는 값이 아닌 살아있는 수식 — 소계 =SUM(구간), 상단합계/총합계 =소계셀들의 합
- [ ] 2기 시즌 전: 합계표 selectbox3에 "2기 …" 라벨 노출 확인, 현금영수증 X 업체
      상반기 섞임 처리(거래년월 파싱) 검토
- [ ] 예정신고 시즌(4월/10월) 실사용 검증
