...
| v1.0.6 | ① 통합조회 '조회권한이 없습니다' alert 예외처리 — 즉시 '출력 생략' 성공(노란 로그), 업체는 계속 진행 (해결 #12) (최신) |
...
- [ ] **납부서 출력(payment_slip) 라이브 검증** — 2026-07-22 구현, 미검증.
      신고내역 모달의 요소를 id가 아닌 텍스트 탐색으로 잡아 data-vap 속성 태깅
      (입력칸=라벨 조상 검색, 조회 버튼, 기간 프리셋, 행별 마지막 '보기'=납부서 열).
      알림("조회가 완료되었습니다"/"…확인하세요")은 **네이티브 alert가 아니라
      WebSquare DOM 모달** — _dismiss_notice가 감지+확인 클릭. 검증 포인트:
      ① 모달 태깅 성공 여부 ② 알림 감지 ③ '보기' 2개 중 납부서 선택
      ④ 뷰어 팝업 인쇄→PDF 파이프라인 재사용 동작 ⑤ 0건/납부서 없음 분기.
