...
- 업체 명부: 인라인 추가줄(업체명/사업자번호/예정 O·X/+추가, Enter 지원).
  ...
  **체크된 업체만 실행**(자료/납부서 공통). 큰 명부를 유지하며 그때그때 골라 실행.
  모두 체크/해제, 체크 삭제 버튼. checked는 clients.json에 저장(없으면 체크 간주).
...
| v1.1.0 | 납부서 출력 모드 — 신고내역 조회(접수증·납부서)에서 업체별 최신 1건 납부서 PDF, 파일명 템플릿({업체명}/{납부기한}+날짜토큰), 모달 재사용, 번호일치 안전장치 (해결 #13·#14, 실업체 2곳 라이브 검증) (최신) |
