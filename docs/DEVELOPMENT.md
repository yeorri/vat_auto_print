...
| 12 | ① 통합조회 40초 timeout "실패"(수임은 정상) | 일부 업체(지점)는 조회 응답이 통째로 빈 화면 → 완료 신호(과세기간 칸)가 영영 안 옴 | timeout 후 기본사항 칸 전부 공란이면 '결과 없음 — 출력 생략' 성공 처리 (v1.0.6) |
...
| v1.0.6 | ① 통합조회 빈 화면 응답 예외처리 — '결과 없음' 성공 처리 (해결 #12) (최신) |
