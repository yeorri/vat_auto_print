...
### 납부서 출력 (별도 모드 — GUI 상단 [자료 출력|납부서 출력] 전환)

| KEY | 내용 | 상태 |
|-----|------|------|
| payment_slip | 신고/납부>부가가치세>신고내역 조회(접수증·납부서) — 업체별 최신 1건 납부서 PDF | ✅ 라이브 검증 완료 (v1.1.0, 실업체 2곳 연속) |

- 파일명 템플릿: `{업체명}`/`{납부기한}` 치환 + 날짜형식 토큰(YYYY/YY/MM/DD).
  기본 `[납부서]부가가치세_{업체명}_{납부기한}` + `YY.MM.DD` → `…_26.07.27.pdf`
- 납부기한은 GUI에서 실행 전 한 번 입력. 항상 PDF 저장(인쇄 없음), 업체별 하위 폴더.
- 이 모드는 명부의 '예정신고' 열이 없어도 실행 가능. 사업자번호 오류 알림
  ("…확인하세요" DOM 모달)은 해당 업체만 건너뛰고 기록.
- 모달 재사용: 2번째 업체부터 메뉴 재진입 없이 번호만 바꿔 이어서 조회(업체당 ~8초).
  그리드 행은 사업자번호 열 일치로만 선택 — 잔여/전체 목록이 떠도 오출력 불가.
- ⚠ 저장 다이얼로그가 뜨는 순간 포커스 독점 프로그램(세무사랑 등)이 화면을 잡고
  있으면 Chrome '인쇄 실패' — phase가 [보기]부터 1회 자동 재시도.
